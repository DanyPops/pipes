...
function fakePi(): {
	...
	const fire = async (event: string, ctx: any = {}) => {
		// Real Pi's ExtensionContext always carries a sessionManager and isIdle() -- defaulted here
		// (isIdle: () => true, matching @danypops/pi-extension-harness's own stub) so a test that
		// doesn't care about turn/session state (most of them) doesn't need to repeat this everywhere.
		const fullCtx = { sessionManager: { getSessionId: () => "test-session" }, isIdle: () => true, ...ctx };
